...
public interface AppointmentRepository extends CrudRepository<Appointment, UUID> {
    ...
    @Transactional
    @Modifying
    @Query("DELETE FROM Appointment a WHERE a.customer IN (SELECT c FROM Customer c WHERE c.username = :customerName)")
    void deleteAllAppointmentsByCustomerUsername(@Param("customerName") String customerName);
